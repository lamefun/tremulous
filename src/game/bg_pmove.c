...
==============
*/
#define WALLCOAST_GROUNDED qtrue
#define WALLCOAST_3D qfalse
...
==============
*/
static void PM_WallCoast( vec3_t wishDir, qboolean grounded )
{
...
static qboolean PM_CheckJump( vec3_t customNormal )
{
  ...
  // Don't allow automatic jumping to happen too often.
  if( ( pm->ps->pm_flags & PMF_JUMP_HELD ) &&
      ( ( pm->ps->pm_flags & PMF_TIME_WALLJUMP ) ||
        ( pm->ps->pm_flags & PMF_TIME_LAND ) ) )
    return qfalse;
