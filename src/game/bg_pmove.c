...
==============
*/
static void PM_AccelerateFull( vec3_t wishdir, float wishspeed, float accel,
                               qboolean novertical )
{
  float vertspeed;     /* Vertical speed (if temporarily removed from
                          the actual velocity). */
  ...
  /* Temporarily remove the vertical speed. */
  if ( novertical )
  {
    vertspeed = pm->ps->velocity[ 2 ];
...
Handles user intended acceleration, allowing vertical acceleration.
==============
*/
#define PM_Accelerate( wishdir, wishspeed, accel ) \
  PM_AccelerateFull( wishdir, wishspeed, accel, qfalse )
...
==============
*/
#define PM_AccelerateHorizontal( wishdir, wishspeed, accel ) \
  PM_AccelerateFull( wishdir, wishspeed, accel, qtrue )
...
{
  vec3_t currentdir; /* Current direction (horizontal). */
  vec3_t redirected; /* New horizontal velocity of the player. */
  ...
  /* Don't redirect momenum unless moving forward. */
  if( pm->ps->movementDir != 0 || wishspeed == 0.0f )
    return;
...
static qboolean PM_CheckJump( void )
{
  ...
  float maxforwardspeed; /* Maximum forward speed to gain from jumping. */
  float forwardfactor;   /* Fraction of the jump speed to additionally add to
                            the forward speed. */
  int i;                 /* Loop index. */
  ...
  /* Add some forward speed as well, but limit the maximum speed. Returns from
  subsequent jumps are diminishing, to prevent players from accelerating too
  much too quickly. */

  PM_GetDesiredVelocitySimple( wishdir );
  maxforwardspeed = VectorNormalize( wishdir );
  goodspeed = DotProduct( pm->ps->velocity, wishdir );

  if ( PM_IsMarauder( ) )
  {
    maxforwardspeed *= 3.0f;
    forwardfactor = 0.7f;
  }
  else
  {
    maxforwardspeed *= 1.75f;
    forwardfactor = 0.3f;
  }

  if ( maxforwardspeed >= 1.0f && goodspeed < maxforwardspeed )
  {
    float speedfactor = 1.0f - goodspeed / maxforwardspeed;
    float pushfactor = speedfactor * 0.7f + 0.3f;
    float maxpush = maxforwardspeed - goodspeed;
    float push;
    push = MIN( jumpMagnitude * forwardfactor * pushfactor, maxpush );
