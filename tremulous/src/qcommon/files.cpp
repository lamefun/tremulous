...
{
    char defaultBasepath[MAX_OSPATH];

    Com_Printf("----- FS_Startup -----\n");
    fs_packFiles = 0;

    #ifdef INSTDIR_DATA
      Q_strncpyz( defaultBasepath, INSTDIR_DATA, sizeof( defaultBasepath ) );
    #else
      Q_strncpyz( defaultBasepath, Sys_DefaultInstallPath( ),
                  sizeof( defaultBasepath ) );
      Q_strcat( defaultBasepath, sizeof( defaultBasepath ), "/data" );
    #endif
    ...
    fs_gamedirvar = Cvar_Get("fs_game", PRIMARY_MOD, CVAR_INIT | CVAR_SYSTEMINFO);
...
void FS_InitFilesystem(void)
{
    ...
    if (!FS_FilenameCompare(Cvar_VariableString("fs_game"), PRIMARY_MOD))
      Cvar_Set("fs_game", PRIMARY_MOD);

    // try to start up normally
    FS_Startup(PRIMARY_MOD);
